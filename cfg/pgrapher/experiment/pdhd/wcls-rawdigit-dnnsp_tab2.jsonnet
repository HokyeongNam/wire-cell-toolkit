...
local trad_dump = g.pnode({ type: 'DumpFrames', name: 'spsink' }, nin=1, nout=0);
local trad_branch = if save_tradsp then g.pipeline([wcls_output.sp_signals, trad_dump], 'tradsp') else null;
local trad_tap = if save_tradsp then g.fan.tap('FrameFanout', trad_branch, name='tradsp_tap') else null;

local dnn_retagger = g.pnode({
  type: 'Retagger',
  name: 'dnnout',
  data: {
    tag_rules: [{
...
local graph = g.pipeline(
  [wcls_input.adc_digits, fanpipe, sp_retagger]
  + (if save_tradsp then [trad_tap] else [])
  + [dnn_retagger, wcls_output.dnn_signals, sink]
);
