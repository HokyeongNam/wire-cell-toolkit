...
local nchunks = 1;
local nticks_tde = 6400;
local nticks_bde = 6250;
// local model = "ts-model/unet-haiwang.ts"; // mobilenetv3-hokyeong.ts, unet-haiwang.ts
local model = "ts-model/CP49_mobilenetv3_rebin10_lr0p1_thr100_U.ts";
// local model = "ts-model/CP49_mobilenetv3_rebin4_lr0p1_thr100_U.ts";
// local model = "ts-model/unet-cosmic390-newwc-depofluxsplat-pdhd.ts";
...
local nfsp_pipes = [
  g.pipeline(
             ...
             + (if use_dnnroi then [
                dnnroi(tools.anodes[n], ts, output_scale=1.0, nticks_tde=nticks_tde, nticks_bde=nticks_bde, nchunks=nchunks),
                mio.dnnsp_pipe[n],
               ] else []),
             'nfsp_pipe_%d' % n)
  for n in std.range(0, std.length(tools.anodes) - 1)
